...
#show: ceel.with(
  ...
  keywords: ("Bitboards", "Geração de lances", "Inteligência artificial", "Rust", "Xadrez"),
  abstract-en: [
    The application of artificial intelligence to chess depends on data structures and algorithms capable of representing the board and generating moves with low computational cost. This paper presents ChessLib, a chess library implemented in Rust for efficient game-state representation and move generation based on bitboards and _magic bitboards_. The solution explores bitwise operations, precomputed attack tables, and a modular organization oriented toward extensibility, with emphasis on performance and memory safety. In addition to describing the adopted architecture, the paper defines an experimental evaluation protocol based on _Perft_ tests and comparisons with reference libraries in the domain.
  ],
  keywords-en: ("Artificial intelligence", "Bitboards", "Chess", "Move generation", "Rust"),
)
